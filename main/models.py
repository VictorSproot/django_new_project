...
class Rubric(models.Model):
    ...
    slug = models.SlugField(max_length=200, db_index=True, verbose_name='Адрес')                        
...
class AuthorBook(models.Model):
    title = models.CharField(max_length=200, db_index=True, verbose_name='Автор')
...
class Bb(models.Model):
    ...
    slug = models.SlugField(max_length=200, db_index=True, verbose_name='Адрес')
    author_book = models.ManyToManyField('AuthorBook', verbose_name='Автор')
    content = RichTextField(blank=True, db_index=True, verbose_name='Описание')
    lang_category = models.IntegerField(choices=CATEGORIES, default=1, db_index=True, verbose_name='Язык')
    image = models.ImageField(blank=True, upload_to=get_timestamp_path,
                              verbose_name='Изображение')
    book_file = models.FileField(upload_to=get_timestamp_path, null=True, blank=True, verbose_name='Файл PDF')                          
    author = models.ForeignKey(AdvUser, on_delete=models.CASCADE,
                               verbose_name='Добавил книгу')
    release_date = models.IntegerField(db_index=True, verbose_name='Год выхода')                          
